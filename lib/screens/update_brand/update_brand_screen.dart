...
class UpdateBrandScreen extends StatefulWidget {
  ...
  const UpdateBrandScreen({Key? key, required this.mainStore, required this.phoneBrandModel}) : super(key: key);

  @override
  _UpdateBrandScreenState createState() => _UpdateBrandScreenState();
}
...
class _UpdateBrandScreenState extends State<UpdateBrandScreen> {
  ...
  void onUpdate() {
    ...
    if (imageValidate && _phoneBrandController.text.isNotEmpty) {
      imageService.insertImage(brandAttachmentsList, widget.phoneBrandModel.imageIdRef);

      imageService.deleteImage(deletedImage);

      if (_phoneBrandController.text.isNotEmpty) {
        Map<String, dynamic> postData = {
          'id': widget.phoneBrandModel.id,
          'name': _phoneBrandController.text,
        };
        phoneBrandService.updatePhonebrand(postData).then((value) {
          AwesomeDialog(
          ...
          widget.mainStore.phoneBrandStore.loadData().whenComplete(() {
            for (var brand in widget.mainStore.phoneBrandStore.phoneBrandList) {
              if (brand.id == widget.phoneBrandModel.id) {
                Future.delayed(Duration(seconds: 2)).whenComplete(() {
                  Navigator.pop(context);
                  Navigator.pop(context);
                  Navigator.of(context).pushReplacement(
                    MaterialPageRoute(builder: (context) {
                      return ListPhoneByCategory(mainStore: widget.mainStore, phoneBrandModel: brand);
                    }),
                  );
                });
  ...
  void initState() {
    ...
    super.initState();
    _phoneBrandController.text = widget.phoneBrandModel.name;
  }
      ...
      children: [
        Container(
          ...
          child: CachedNetworkImage(
            ...
            fit: BoxFit.fill,
            imageUrl: baseUrl + widget.phoneBrandModel.images.image,
        ...
        GestureDetector(
          onTap: () {
            setState(() {
              deletedImage.add(widget.phoneBrandModel.images);
            });
          },
          child: Icon(
